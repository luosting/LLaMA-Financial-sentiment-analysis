...
class SFTData(torch.utils.data.Dataset):
    def __init__(
        ...
        #labelmap = {'positive': 'A', 'negative': 'B', 'neutral': 'C'} #mod
        labelmap = {'positive': 0, 'negative': 1, 'neutral': 2} #mod
        ...
        with open(data_file) as fp:
            ...
            for l in lines:
                ...
                #token_q = tokenizer.encode(f'{B_INST} {question} {E_INST}', bos=True, eos=False) # mod
                token_q = tokenizer.encode(f'{question}', bos=True, eos=True) # mod
                #token_a = tokenizer.encode(f'{answer}', bos=False, eos=False) #mod
                token_a = [answer] #mod
                size_q = len(token_q)
                size_a = len(token_a)
    ...
    def next(self):
        ...
        for k in range(self.dp_world_size):
            ...
            num_to_fetch = self.max_seq_len # mod
            seq_len = []
            cnt_train_entries = 1 # mod
            while num_to_fetch > 0:
                # construct X, and seq_len.
                if self.read_pos < len(self.train_data) and num_to_fetch >= self.train_data[self.read_pos][1] and cnt_train_entries <= 1:
                    # fill X with one training entry.
                    tokens, size_qa, size_q, size_a = self.train_data[self.read_pos]
                    X += tokens[:sizeq]    # mod
                    Y.append(tokens[-1])       # mod
                    seq_len.append(size_q) # mod
                    num_to_fetch -= size_q # mod
                    self.read_pos += 1
                    cnt_train_entries += 1
                elif num_to_fetch > 0:
                    # cannot fit or data done: using pad for X.
                    X = [0 for _ in range(num_to_fetch)] + X # mod
                    #Y += [self.tokenizer.pad_id for _ in range(num_to_fetch)] # mod
                    seq_len = [num_to_fetch] + seq_len
                    num_to_fetch = 0 # mod
            ...
            #     tokens, size_qa, size_q, size_a = self.train_data[self.read_pos]
            #     num_to_fetch -= size_q
            #     X = [0 for _ in range(num_to_fetch)] + tokens[:size_q]
            #     Y.append(tokens[-1])
            #     self.read_pos += 1
            #     seq_len = [num_to_fetch, size_q]
            #     if len(Y) == 0:
            #         Y = [-1]                                                                           # mod

            # assert len(X) == len(Y)                                                                 # mod
    ...
    def run(self):
        while True:
            ...
            for batch_index in range(self.micro_batch_size):
                ...
                # assert len(data[1]) == self.max_seq_len, f'{len(data[1])} vs {self.max_seq_len}' # mod
